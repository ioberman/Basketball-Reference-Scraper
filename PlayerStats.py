...
df1['Merge Field'] = df1['year_start'].astype(str) + df1['Player']
...
final_columns_to_remove = ['Merge Field','Lg','year_start','MP_y','PTS_y','TRB_y','AST_y','STL_y','BLK_y','FG%_y','3P%_y','FT%_y','Player_x']

NBA_df = NBA_df.drop(columns=final_columns_to_remove, axis=1)

#rename some columns
NBA_df = NBA_df.rename(columns={ 'FG%_x':'FG%', '3P%_x':'3P%', 'TRB_x': 'TRB', 'FT%_x': 'FT%', 'AST_x': 'AST', 'STL_x': 'STL','BLK_x': 'BLK', 'PTS_x': 'PTS','MP_x':'MP','Player_y':'MVP Name'})
#shows df
NBA_df
...
NBA_df.to_csv('Documents/HistoricalNBADataFinal3.csv', index=False)
